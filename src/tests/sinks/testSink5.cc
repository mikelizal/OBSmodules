...
void testSink5::handleMessage(cMessage *msg){
	if ((dynamic_cast <OBS_BurstControlPacket *> (msg) != NULL)&&(msg->getKind() == 3)&&(dynamic_cast <OBS_BCPControlInfo *> (msg->getControlInfo()) != NULL)){
		...
		outputFile << simTime() << "\t" << bcp->getBurstifierId() << "\t" << bcp->getNumSeq() << "\t" << bcp->getSenderId() << "\t" << bcp->getBurstArrivalDelta() << "\t" << bcp->getBurstColour() << "\t" << bcp->getLabel() << "\t" << bcp->getBurstSize() << "\t" << bcp->getByteLength() << "\t" << info->getPort() << "\t" << info->getBCPArrival() << endl;
	}
	else{
		cout << "<testSink5><" << simTime() << "> WARNING: Received a message which is not an electrical OBS_BurstControlPacket (kind 3, OBS_BCPControlInfo attached)." << endl;
	}
	delete msg;
}
